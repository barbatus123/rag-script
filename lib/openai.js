...
export async function uploadFile(content) {
  ...
  const { data } = await client.post('/files', form, { headers });
  return data.id;
}
...
export async function getRecentEmbeddingRequests(offsetMins = 0) {
  const cutoff = Date.now() - (24 * 3_600_000 - offsetMins * 60_000);
